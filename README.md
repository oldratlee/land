...
- [📚 相关资料](#-%E7%9B%B8%E5%85%B3%E8%B5%84%E6%96%99)
    ...
    - [🔖 二手资料](#-%E4%BA%8C%E6%89%8B%E8%B5%84%E6%96%99)
...
👉 一个简单的基于`ClassLoader`用于依赖隔离的容器实现。  
\# 在`Java`中依赖主要是`Jar`。
...
> 结果`Car`引用的类`Wheel`和子直接使用的类`Wheel`的`ClassLoader`不同，即类型不兼容，看起来正确地赋值会抛出的`ClassCastException`！
...
按上面说明的2层委托关系约定，嵌套推广即可得到 包含 **任意层**`ClassLoader`的完备委托关系。✨
